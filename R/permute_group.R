...
brainGraph_permute <- function(densities, resids, N=5e3, perms=NULL, auc=FALSE,
                               ...
                               clust.method='louvain', mat.type=c('positive', 'abs')) {
  Group <- NULL
  ...
  if (isTRUE(weighted)) {
    ...
    res.perm <- switch(level,
                       vertex=permute_vertex_foreach_weighted(perms, densities, resids, groups, measure, diffFun,
                                                              xfm.type = xfm.type, mat.type = mat.type),
                       other=permute_other_foreach_weighted(perms, densities, resids, groups, .function, mat.type = mat.type),
                       graph=permute_graph_foreach_weighted(perms, densities, resids, groups, atlas, auc,
                                                            xfm.type = xfm.type, clust.method=clust.method, mat.type = mat.type))
  } else {
    graphtype = 'binary'
    res.perm <- switch(level,
                       vertex=permute_vertex_foreach(perms, densities, resids, groups, measure, diffFun, mat.type = mat.type),
                       other=permute_other_foreach(perms, densities, resids, groups, .function, mat.type = mat.type),
                       graph=permute_graph_foreach(perms, densities, resids, groups, atlas, auc, mat.type = mat.type))
  }
...
make_graphs_perm_abs <- function(densities, resids, inds, groups) {
...
make_graphs_perm_weighted_abs <- function(densities, resids, inds, groups) {
  corrs <- lapply(unique(groups), function(x)
    corr.matrix.abs(resids[which(groups[inds] == x)],
                    densities=densities, rand=TRUE))
  sapply(corrs, lapply, function(x)
